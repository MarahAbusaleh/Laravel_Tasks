            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-10">
                        ...
                        <br>
                        <form 
                        {{-- when Submit a form it must go to store() --}}
                        action="{{ route('Category.update', $category->id) }}"
                        {{-- POST becuse we want to save a date in DB --}}
                        method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        {{-- converte the POST method into PATCH method --}}
                        @method('PATCH')
                            <div class="mb-3">
                                <label for="addedCateroryName" class="form-label">Name :</label>
                                <input type="text" class="form-control" id="addedCateroryName" name="name" value="{{ $category->name }}">
                            </div>
                            <div class="mb-3">
                                <label for="addedCateroryDesc" class="form-label">Description :</label>
                                <input type="text" class="form-control" id="addedCateroryDesc" name="desc" value="{{ $category->desc }}">
                            </div>
